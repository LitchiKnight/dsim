...
class InitCmd(BaseCmd):
  ...
  def pull_from_git(self) -> None:
    ...
    status, err_msg, _ = self.run_cmd("git --version", mask=True)
  ...
  def pull_from_svn(self) -> None:
    ...
    status, err_msg, _ = self.run_cmd("svn --version", mask=True)
    if status != CmdStatus.PASS:
      Utils.error("SVN is not installed")
    os.chdir(work_path)
    status, err_msg, _ = self.run_cmd(f"svn checkout {repo_path}")
    if status == CmdStatus.PASS:
      Utils.info(f"Pull remote repository from {repo_path}")
    else:
      Utils.error(err_msg)
